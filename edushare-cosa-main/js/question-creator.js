(function () {
    ...
    // -------- NEW: AI page target
    const AI_URL = 'AI-Question-Creator.html';
    ...
    if (ctxEl) ctxEl.textContent = `Play: ${play} • Act: ${act} • Scene: ${scene}`;
    ...
    // -------- NEW: AI button (top-right)
    const btnAI = document.getElementById('btnAI');
    if (btnAI) {
        // Tooltip (mouse hover)
        if (!btnAI.getAttribute('title')) {
            btnAI.setAttribute(
                'title',
                '✨ Create questions with AI — pick type, amount, and act/scene in seconds.'
            );
        }

        // Redirect to new page with context
        btnAI.addEventListener('click', () => {
            const qs = new URLSearchParams({ play, act, scene }).toString();
            window.location.href = `${AI_URL}?${qs}`;
        });
    }
